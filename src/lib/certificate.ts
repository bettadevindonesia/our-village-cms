...
// Define a type for the certificate data based on the schema
...
export async function getAllCertificates(): Promise<Certificate[]> {
  try {
    const result = await db.select().from(certificates).orderBy(desc(certificates.createdAt)); // Newest first
    return result;
  } catch (error) {
...
export async function deleteCertificate(id: number): Promise<boolean> {
  try {
    ...
    revalidatePath('/dashboard/certificates'); // Refresh the list page
    return true;
  } catch (error) {
